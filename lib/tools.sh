...
tissCustomInstall() { # tissCustomInstall <tool> -> install command for tools
  ...
  # shellcheck disable=SC2016  # these are shown-then-run verbatim by ensureTool; $/$() must stay literal
  case "$1" in
    ...
    session-manager-plugin) echo 'smp_arch=64bit; case "$(uname -m)" in aarch64 | arm64) smp_arch=arm64 ;; esac; if command -v brew >/dev/null 2>&1; then brew install --cask session-manager-plugin; elif command -v dpkg >/dev/null 2>&1; then curl -fsSL "https://s3.amazonaws.com/session-manager-downloads/plugin/latest/ubuntu_${smp_arch}/session-manager-plugin.deb" -o /tmp/session-manager-plugin.deb && sudo dpkg -i /tmp/session-manager-plugin.deb; elif command -v yum >/dev/null 2>&1 || command -v dnf >/dev/null 2>&1; then curl -fsSL "https://s3.amazonaws.com/session-manager-downloads/plugin/latest/linux_${smp_arch}/session-manager-plugin.rpm" -o /tmp/session-manager-plugin.rpm && sudo yum install -y /tmp/session-manager-plugin.rpm; else echo "SessionManagerPlugin needs a platform-specific installer — see https://docs.aws.amazon.com/systems-manager/latest/userguide/session-manager-working-with-install-plugin.html" >&2; exit 1; fi' ;;

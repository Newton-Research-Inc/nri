# tiss tf deep-diff report (jq program) — ported VERBATIM from Matt's
# tools/tf/plan-report-md.sh. Markdown for PR descriptions / any renderer.
# One input document per .tfplan.json; run via:
#   tiss tf report --format diff [plan.json...]
#
# Layout: modules as nested <details open>, one level per module; each
# resource is a ```diff fence with aligned attribute trees, embedded-JSON
# decoding, name-keyed arrays (inserts don't cascade), (known after apply),
# and `# forces replacement` flags.

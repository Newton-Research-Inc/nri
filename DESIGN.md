...
One metaphor, three levels — every term earns its meaning:

- **leaf** — one command script (polyglot: bash, python, anything executable).
- **tree** — one overlay: a directory whose `scripts/` of leaves is the
  command language. A **package** is a tree distributed via git (branch
  `tiss/<name>` — the branch contains a tree).
- **pile** — the ordered stack of trees. Things get tossed on top
  (`tiss +devops`), and the top wins: tiss routes your command through
  the pile, and the first tree that has it, wins. Core is always the
  bottom of the pile.
...
| `tissShims`, `tissShortcutList/Lookup` | done | Shortcut shim dir + merged `name = command words` definitions (user file, then tree etc/shortcuts) backing argv[0] shortcut routing (see decision log) |
...
| Date | Decision |
| --- | --- |
| 2026-08-04 | `tissOfferRcActivation` fixed a fresh-box chicken-and-egg (Matt hit it on a new machine): a rc with only `eval "$(tiss init)"` and no `$HOME/.local/bin` on PATH is unrecoverable at shell startup — you need `tiss` resolvable to run that eval in the first place, so the PATH guard baked into `tiss init`'s own emitted output never gets a chance to run. Fix: the PATH line and the eval line are now checked and healed independently (no single marker gate), and when the PATH line is missing it's *prepended* to the rc — via write-through (`cat > `, not `mv`, so a symlinked dotfile stays a symlink) — rather than appended, since an existing/orphan eval line may already be earlier in the file. Self-heals boxes already bitten by the old append-only version, not just new installs |
| 2026-08-04 | ajl lazy-install reinstated (Matt): `tissCustomInstall` gets an `ajl` entry (`uv tool install git+https://github.com/mattyo161/ajl`, no PyPI release yet) and `ajl` re-joins `TISS_INSTALL_ALLOW_DEFAULT` — install-only, no tiss-native handler. Does not reopen the 2026-07-18 call below: ajl still carries none of its own routing/caching logic in core, it's just installable again |
| 2026-07-17 | `tiss self X` legacy spelling killed, as flagged "kill at 1.0" in the 2026-07-15 reserved-lexicon entry below — now that release-please cut v1.0.0. `self/` is no longer routable as a namespace by any means: resolveInTree refuses to descend into it (execution), the bare-namespace-landing check excludes it (`tiss self` no longer shows help), and shellComplete refuses to descend into it even when asked by name (`tiss --complete self`). It behaves exactly like any other unmatched top-level word (passthrough, `'self' is not installed`). The reserved lexicon's fast path (`findScript "$TISS_SCRIPTS/self" "$word"`) is untouched — it never went through `resolveInTree`, so `tiss doctor`/`tiss init`/etc. are unaffected; `scripts/self/*.sh` stays as their private backing store. Fallout fixed: `~/.zshrc.local`'s `eval "$(tiss self init)"` → `tiss init`; several tests had never migrated off the legacy spelling as their calling convention (test_config.sh, test_env.sh, test_tools.sh, test_shortcuts.sh) — updated to the top-level words; test_dispatcher.sh's generic namespace-help/completion-descent tests, which used `self` merely as a stand-in example namespace, switched to `ssm` |
| 2026-07-16 | tf leaves core for the **devops tree** (branch `tiss/devops`, tagged v0.1.0) — the first real package, scaffolded by the new `tiss pile new NAME [--push]`. The tree carries scripts/tf, lib/tf*.sh (tf.sh via the tree's lib/init.sh; plan.sh sources tf-parallel tree-relatively, NOT via $TISS_LIB which is always core), the TISS_TF_* defaults as ACTIVE cfg lines in the tree's etc/config.sh (user config still wins), suggested shortcuts, and its own tests — `tiss test` now runs every enabled tree's tests/test_*.sh (tree tests source the core harness via TISS_HOME and put their own tree on TISS_PATH). Core keeps the tf→terraform passthrough alias: without the tree, `tiss tf plan` is plain terraform; with it, the wrapper. Install: `tiss +devops` |
| 2026-07-15 | REVERSED (Matt challenged it): meta-commands leave the `self` namespace for a top-level **reserved lexicon** (`checkout completion config doctor env help init pile pull shell shortcuts test version`) — the survey (git/gh/terraform/mise/brew/kubectl) shows top-level meta + protected builtins IS the convention; rustup's `self` is the outlier. The lexicon is a security contract, not just UX: the dispatcher resolves these words from core's scripts/self/ BEFORE the pile walk, so no tree can shadow `pull`/`doctor` (previously an overlay could hijack them — supply-chain vector); doctor warns when a tree ships a reserved name, shortcuts refuse reserved names. `tiss -- <cmd>` escapes the lexicon to the real binary. Help leads with the lexicon then `== Commands ==` (terraform's main/other shape); completions/manifest carry reserved entries; `tiss self X` still works, hidden, kill at 1.0. `cd` was killed outright (Matt: never used) along with the rc wrapper function that existed only to serve it. Collision audit: only the real `tree` binary mattered — solved by the pile rename below |
| 2026-07-15 | Vocabulary: the overlay stack is the **pile** (see Vocabulary section) — `tree` was doing double duty for unit AND collection. `tiss pile add\|remove\|list\|resolve [--json]`, jsonl resolution surfaces ("where does this name fetch from"), name->(repo,branch) mapping lives IN THE CLONE (origin + tiss.track, refspec self-heal for older clones) — no registry file to drift. Considered: `skid` (bad connotations, no mechanics), `leaf` (already means a command script — instead it completes the family: pile ⊃ trees ⊃ leaves), `branch` (hopelessly overloaded). `pile` won on mechanics: top wins is literally how piles work. Frees `tiss tree` back to the real tree binary |
| 2026-07-15 | Empty-box bootstrap (Matt hit the dead end in AWS CloudShell): mise is first-class — `ensureTool mise` runs the official `curl https://mise.run \| sh` (same TISS_AUTO_INSTALL gating), then activates in-process (~/.local/bin + mise shims onto exported PATH) so the command originally typed still succeeds with zero shell setup. brew is guided — found-but-unactivated brew gets `shellenv`'d in-process; missing brew prints the exact installer + platform shellenv lines (never auto-run: sudo/Xcode CLT is too invasive). Persistent activation rides the existing single rc line: `tiss self init` now also emits ~/.local/bin PATH guard, brew shellenv fallback and mise activation (all guarded no-ops when already set up) — tiss never edits rc files itself, same emit-don't-edit stance as completions. Verified end-to-end in a bare amazonlinux:2023 container |
| ~~2026-07-15~~ | ~~ajl integration (github.com/mattyo161/ajl, Matt's boto3-as-jsonl streamer — jsonl-native, so a first-class tiss citizen): `ajl/_self.sh` handler reuses the ssm intent-routing pattern — `<svc> get-*/describe-*/list-*` reads fronted by cacheExec (TISS_AJL_CACHE_DURATION 1h, TISS_AJL_CACHE_ENCRYPT 0), everything else narrated via learnExec and never cached; `--params-json` streams also never cached (stdin drives the call, argv can't key it).~~ REVERSED 2026-07-16 (Matt): the caching/narrating wrapper doesn't belong in tiss core — if ajl wants a tiss-native handler it should ship one itself (e.g. via a pile package), not have tiss carry ajl-specific routing logic. Handler + tests removed; ~~`tiss ajl ...` now falls through to plain passthrough (still lazy-installable via `tissCustomInstall`, unaffected by this revert)~~ FURTHER REVERSED 2026-07-18: the dormant `tissCustomInstall` ajl entry and its `TISS_INSTALL_ALLOW_DEFAULT` slot removed too — ajl is fully out of tiss core now, no lazy-install special-casing left |
| 2026-07-14 | Shortcuts: muscle-memory names (`tfplan`, `saveData`, ...) are symlink shims in TISS_SHIMS pointing back at the dispatcher, routed by argv[0] — no generated stub code, nothing to go stale. Definitions in ~/.config/tiss/shortcuts + tree etc/shortcuts (`name = command words`; user > specific tree > core, first wins), managed by `tiss self shortcuts add\|remove\|list\|sync`; `self init` also emits the guarded PATH line. The dispatcher strips TISS_SHIMS from PATH at startup, so passthrough and child scripts only see real commands — recursion through a shim is structurally impossible and shortcuts never leak into script composition. Expansion applies only when $0 lives in the shim dir (a symlink elsewhere stays a plain alias); a leading @env hoists ahead of the expansion. sync only ever touches symlinks that point at a */bin/tiss; foreign files are reported, never removed. Rejected alternatives: generated stub scripts (stale expansions, second exec path), `eval "$(tiss shortcuts)"` functions (invisible to scripts/xargs/cron). Deferred: per-shortcut completions, manifest type:shortcut entries |
...
| 2026-07-14 | tf wrappers rebuilt from Matt's production tools/tf: self-correcting init (auth-expiry pause-retry, backend-change -reconfigure), versioned artifacts under .tiss/tfplans/ (tfplan+log+json+run.json, latest.json), plan summary uses single-width ICONS (✚✎✘⟳⊘↧➜) so column-selection yields a -target list, apply-from-metadata never re-plans (stale detection, already-applied warning, applied_at recorded), prompt via TISS_TF_AUTO_APPLY=ask\|always\|never or -y/--no-apply, `tf report --format list\|target\|md\|tree\|csv\|jsonl\|excel` reads saved json (terraform not needed). Only .tfplan self-destructs (TISS_TF_PLAN_TTL, 0=keep); json/log/meta persist for reports. Follow-up 2026-07-15: --all/--skip-fresh/dir args/`--` separator, -j/--parallel ported (lib/tf-parallel.sh: worker status files, live tty region, credential-expiry pause/re-queue with single retry, full-counts completion blocks, summary table; plan-only by design), deep-diff drift report ported VERBATIM as lib/tf-diff.jq (`tf report --format diff`), apply --all. Root-module discovery pattern configurable (TISS_TF_MODULE_GREP, default any `backend "`) |
| 2026-07-16 | Fresh-box last mile: after a mise bootstrap (and via doctor), tiss OFFERS to append `eval "$(tiss init)"` to the shell rc — bkup'd, idempotent, consent per TISS_AUTO_INSTALL. Reconciled with the other session's bootstrap: theirs won (ensureTool recurses to bootstrap mise, `tiss init` is the single activation line); this adds only the write-it-for-you step Matt asked for |
| 2026-07-14 | Releases automated: conventional commits (CI-linted, dependency-free grep) + release-please (simple type; version.txt + CHANGELOG.md; merging the release PR tags and publishes). `tiss --version` reads version.txt |
